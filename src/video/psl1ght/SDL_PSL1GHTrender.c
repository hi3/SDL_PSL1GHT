                     ...
                     const SDL_Rect * srcrect, const SDL_Rect * dstrect)
{
    ...
    if (SDL_ISPIXELFORMAT_FOURCC(texture->format)) {
        ...
        dst = (Uint8 *) data->screens[data->current_screen]->pixels + dstrect->y *
                        data->screens[data->current_screen]->pitch + dstrect->x *
...
SDL_PSL1GHT_RenderPresent(SDL_Renderer * renderer)
{
    ...
    /* Page flip */
    deprintf (1, "\tPage flip: %d\n", data->current_screen);
    flip(data->context, data->current_screen);

    /* Wait for vsync */
    //if (renderer->info.flags & SDL_RENDERER_PRESENTVSYNC) {
    deprintf (1, "\tWait for vsync: %d\n", data->current_screen);
    waitFlip();
    //}
